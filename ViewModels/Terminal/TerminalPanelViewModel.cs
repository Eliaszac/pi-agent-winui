    ...
    Func<string, string, ITerminalSession>? createScriptSession = null) : ObservableObject, IAsyncDisposable
{
    ...
    public Models.Projects.ExecutionTarget? Target
    {
        ...
        set
        {
            ...
            target = value;
            var matching = Tabs.LastOrDefault(tab => tab.TargetId == value?.Id && tab.ConversationId == ConversationId);
    ...
    public void Show(TerminalTabViewModel tab) { if (Tabs.Contains(tab)) { Selected = tab; IsOpen = true; } }
    ...
    public void Toggle(string directory)
    {
        ...
        else if (Target is null && Tabs.LastOrDefault(tab => tab.ConversationId == ConversationId) is { } local) { Selected = local; IsOpen = true; }
        else if (Tabs.LastOrDefault(tab => tab.ConversationId == ConversationId && tab.TargetId == Target?.Id && tab.Directory == directory) is { } existing) { Selected = existing; IsOpen = true; }
        else Add(directory);
    }
    ...
    public void Add(string directory)
    {
        ...
        var tab = new TerminalTabViewModel(directory, ++sequence, session, target is null ? null : $"{target.Label} · {sequence}") { TargetId = target?.Id, ConversationId = ConversationId };
        Tabs.Add(tab);
        Selected = tab;
        IsOpen = true;
    }
    ...
    public void RunScript(Guid projectId, Guid scriptId, string name, string directory, string command, Models.Projects.ExecutionTarget? executionTarget = null)
    {
        ...
        var key = $"{ConversationId}:{projectId}:{executionTarget?.Id}:{scriptId}";
        ...
        if (active is null)
        {
            ...
            active = new(directory, ++sequence, session, executionTarget is null ? name : executionTarget.Label + " · " + name, key) { TargetId = executionTarget?.Id, ConversationId = ConversationId };
            Tabs.Add(active);
        }
        Selected = active;
        IsOpen = true;
    ...
    public async Task CloseAsync(TerminalTabViewModel tab)
    {
        ...
        if (index < 0) return;
        Tabs.Remove(tab);
        if (Selected == tab) Selected = Tabs.LastOrDefault(item => item.ConversationId == ConversationId);
        if (Selected is null) IsOpen = false;
