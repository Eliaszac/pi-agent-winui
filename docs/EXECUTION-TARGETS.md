...
Conversation artifacts are available for Local Windows, WSL and SSH. `artifact_save` imports an absolute file path from the conversation's target, or creates text directly in Windows-owned artifact storage. Transfer uses the existing SSH authentication/WSL distribution, preserves binary bytes and excludes shell banners. Downloads, previews and management use the durable Windows copy and work offline. Importing does not delete the original target file.

The composer accepts up to eight uploaded files per message, each at most 32 MiB. `artifact_read` reads sent text/image artifacts without a remote connection. `artifact_path` gives other file formats to target-side tools: a managed path on Windows, or a transferred working copy under `$HOME/.pi-desktop-artifacts` on WSL/SSH. Binary transfer uses coreutils and the configured authentication; it does not require Python. Remote edits are not automatically synchronized back—use `artifact_save` to publish them. File deletion attempts remote cleanup; offline cleanup is retried on later artifact file access or deferred conversation deletion. Live WSL/SSH verification remains pending; mocked transfer, command construction, binary framing and target-specific creation have unit coverage.
