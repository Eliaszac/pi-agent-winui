...
- The confirmed public name is Pi desktop; publisher/company metadata is Eliaszac. The About page and legal documents retain Denmark and eliaszacho@gmail.com. Installer support and update links point to this repository's issues and releases. This metadata is not a code signature.
...
- During installation and upgrades, `[InstallDelete]` includes `ObsoleteArtwork.iss`: 63 exact obsolete SVG/PNG paths removed by the artwork replacement in commit `dcbdd3e`. No asset-directory wipe or wildcard deletion is used. Current artwork, dependency notices, unknown files and user data are outside that inventory. Keep historical entries so users can skip intermediate versions.
...
Headless packaging tests validate the cleanup inventory, retained assets and stable identity, and simulate repeated exact-file cleanup in a temporary fixture. They do not execute Inno Setup or prove installed-upgrade behavior. Installer compilation likewise does not verify an actual upgrade.

For installed verification, seed the obsolete filenames from `ObsoleteArtwork.iss` in a disposable older installation, upgrade, and check that only those files disappear. Check current artwork and notices, an unknown sentinel file, and user data remain intact. Repeat with a clean installation and same-version repair. Do not test cleanup against an active user installation without explicit authorization.
