...
export default function registerExecutionTarget(pi: ExtensionAPI): void {
    ...
    pi.on("session_start", () => {
        for (const tool of pi.getAllTools()) if (tool.name.startsWith("embedded_browser_") || ["artifact_save", "artifact_list", "artifact_read", "artifact_path", "github_update_issue", "github_comment_pr"].includes(tool.name)) allowed.add(tool.name);
