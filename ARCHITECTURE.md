...
Reading positions are retained per conversation for the view's lifetime using a message identity and its viewport offset, without keeping UI containers alive. Following uses native `KeepLastItemInView` anchoring; reading history uses `KeepItemsInView`. Only initial/explicit tail navigation realizes a distant final item. Streaming schedules arrangement rather than remeasuring the transcript and consumes each measured downward correction once; it never repeatedly requests `ScrollIntoView` or applies an upward correction from transient row geometry. Focus changes do not automatically bring transcript controls into view. Wheel, scrollbar/panning, and scroll-key input detach following; scrolling down near the measured final row reattaches it, without relying on the estimated virtualized extent. Nested scroll viewers do not change transcript following. Native reproduction/verification of the latest scroll changes remains pending.
