...
- Native command mappings currently cover session details, compact (optional instructions), HTML export through the Windows save picker, model, thinking, copy, new conversation, header rename, and Extensions. Native TUI-only commands are intercepted with an unavailable notice rather than passed to the model. Fork/clone/tree/import, provider management, settings/trust/reload, and sharing require separate native workflows and remain future work.
