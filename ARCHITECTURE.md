...
Usage retains a metadata-only archive in `usage-history.json` beside the project catalog, in addition to the file-metadata memory cache. Atomic replacement and a shared in-process gate serialize archive merges and resets. Response identities deduplicate live, deleted and forked history; no prompt copies or telemetry uploads are added. Deferred cleanup captures final usage before deleting a session, including when Home has never scanned it, and retries if preservation fails. Reads remain bounded to 64 MiB per session, 256 MiB of uncached files per scan, two million characters per JSONL record, and a ten-second budget between files; partial/unreadable records are disclosed. Home refreshes on navigation and every thirty seconds while visible. Hiding Home analytics does not disable retention. Resetting totals physically removes earlier archived samples and persists a cutoff against reimport, without rewriting Pi sessions or provider records.
...
Reading positions are retained per conversation for the view's lifetime using a message identity and its viewport offset, without keeping UI containers alive. Following uses native `KeepLastItemInView` anchoring; reading history uses `KeepItemsInView`. Only initial/explicit tail navigation realizes a distant final item. Streaming schedules arrangement rather than remeasuring the transcript and consumes each measured downward correction once; it never repeatedly requests `ScrollIntoView` or applies an upward correction from transient row geometry. Focus changes do not automatically bring transcript controls into view. Wheel, scrollbar/panning, and scroll-key input detach following; scrolling down near the measured final row reattaches it, without relying on the estimated virtualized extent. Nested scroll viewers do not change transcript following. The user confirmed the remaining input lag and scroll jitter resolved on 14 September 2026. Small following corrections animate without interrupting an active correction, respect reduced motion, and leave explicit navigation instant. Incoming agent updates use low-priority UI dispatch with time- and count-bounded batches to yield to input.
...
Settings includes a Storage category with bounded background scans for local conversation, artifact, research, checkpoint and diagnostic file sizes. Totals exclude remote working copies, browser data and unrelated app files; incomplete scans are labelled. Known crash diagnostics and completed research can be cleared after confirmation. Research cleanup uses coordinator ownership and preserves active, queued, failed and interrupted work. Existing checkpoint management supplies target-specific cleanup, and deferred conversation cleanup can be retried from Settings. Project source files and shared Pi credentials are excluded.
...
The Processes panel lists the selected conversation's local Windows subprocesses, not remote-target processes or the Pi root. Rows show name-based general roles and native icons, distinguish Windows helpers, and disclose executable name, process/parent IDs, and start time in expandable details. Unknown executables retain their names without inferred tasks; command/output inspection is not implemented. Stop remains explicit and confirmed with process-tree termination and data-loss warnings. Process ownership checks are unchanged.
